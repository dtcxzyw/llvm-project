...
Value *InstCombinerImpl::EmitGEPOffsets(ArrayRef<GEPOperator *> GEPs,
                                        GEPNoWrapFlags _, Type *IdxTy,
                                        bool RewriteGEPs) {
  auto Add = [&](Value *Sum, Value *Offset, GEPNoWrapFlags NW) -> Value * {
    if (Sum)
  ...
  GEPNoWrapFlags AddFlags = GEPNoWrapFlags::all();
  GEPNoWrapFlags OneUseAddFlags = GEPNoWrapFlags::all();
  GEPNoWrapFlags OneUseFlags = GEPNoWrapFlags::all();
  for (GEPOperator *GEP : reverse(GEPs)) {
    AddFlags &= GEP->getNoWrapFlags();
    ...
    Value *Offset;
    {
      ...
      if (GEP->hasOneUse()) {
        ...
        OneUseAddFlags &= GEP->getNoWrapFlags();
        OneUseSum = Add(OneUseSum, Offset, OneUseAddFlags);
      ...
      if (OneUseSum)
        Offset = Add(OneUseSum, Offset, AddFlags);
    ...
    Sum = Add(Sum, Offset, AddFlags);
    OneUseSum = OneUseBase = nullptr;
    OneUseFlags = GEPNoWrapFlags::all();
    OneUseAddFlags = GEPNoWrapFlags::all();
  }
  if (OneUseSum)
    Sum = Add(Sum, OneUseSum, AddFlags);
  if (!Sum)
