...
; CHECK-NEXT:    [[FABS:%.*]] = call nnan ninf nsz double @llvm.fabs.f64(double [[X:%.*]])
; CHECK-NEXT:    ret double [[FABS]]
;
  %lezero = fcmp ole double %x, 0.0
...
; CHECK-NEXT:    [[FABS:%.*]] = call nnan ninf nsz half @llvm.fabs.f16(half [[X:%.*]])
; CHECK-NEXT:    ret half [[FABS]]
;
  %gtzero = fcmp ogt half %x, -0.0
...
; CHECK-NEXT:    [[FABS:%.*]] = call nnan nsz <2 x double> @llvm.fabs.v2f64(<2 x double> [[X:%.*]])
...
; CHECK-NEXT:    [[FABS:%.*]] = call nnan nsz half @llvm.fabs.f16(half [[X:%.*]])
; CHECK-NEXT:    ret half [[FABS]]
;
