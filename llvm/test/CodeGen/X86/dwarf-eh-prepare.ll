; RUN: opt -mtriple=x86_64-linux-gnu -dwarf-eh-prepare -run-twice < %s -S | FileCheck %s
; RUN: opt -mtriple=x86_64-linux-gnu -passes='require<libcall-lowering-info>,dwarf-eh-prepare' -codegen-opt-level=2 -run-twice < %s -S | FileCheck %s
